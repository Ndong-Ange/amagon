...
import { Store, Eye, EyeOff, Building, User, CheckCircle, X } from 'lucide-react';
...
const SellerRegisterPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        // Stocker les données d'inscription pour le popup
        setRegistrationData(data);
        setShowSuccessPopup(true);
        
        // Auto-fermer le popup après 5 secondes et rediriger
        setTimeout(() => {
  ...
  const handleGoToDashboard = () => {
    if (registrationData) {
      localStorage.setItem('sellerToken', registrationData.token);
      localStorage.setItem('sellerData', JSON.stringify(registrationData.seller));
      navigate('/seller/dashboard');
    }
  };
            ...
            <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    ...
                    <div className="mt-2">
                      ...
                      <div className="mt-4 p-4 bg-green-50 rounded-md">
                        <div className="text-sm">
                          <p className="font-medium text-green-800">Informations de votre compte :</p>
                          <p className="text-green-700 mt-1">
                            <strong>Nom :</strong> {registrationData?.seller?.name}
                          </p>
                          <p className="text-green-700">
                            <strong>Email :</strong> {registrationData?.seller?.email}
                          </p>
                          <p className="text-green-700">
                            <strong>Type :</strong> {registrationData?.seller?.business_type === 'individual' ? 'Particulier' : 'Entreprise'}
                          </p>
                        </div>
                      </div>
                      <p className="text-xs text-gray-500 mt-3">
                        Vous serez automatiquement redirigé vers votre tableau de bord dans 5 secondes...
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <button
                  ...
                  className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-amazon-orange text-base font-medium text-white hover:bg-amazon-orange-hover focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amazon-orange sm:ml-3 sm:w-auto sm:text-sm"
                >
